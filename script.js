...
class Player {
    ...
    draw() {
        if(shieldActive) { ctx.strokeStyle='#0984e3'; ctx.lineWidth=5; ctx.beginPath(); ctx.arc(this.x+35,this.y+35,45,0,Math.PI*2); ctx.stroke(); }
...
function animate() {
    ...
    player.draw();
    if(Math.random() < 0.03 + (level*0.005)) enemies.push({x:Math.random()*(canvas.width-30), y:-50, s:30, spd:3+(level*0.5)});
    if(Math.random() < 0.002) powerups.push({x:Math.random()*(canvas.width-30), y:-50});

    powerups.forEach((p,i) => {
        p.y += 3; ctx.fillText("🛡️", p.x, p.y);
    ...
    enemies.forEach((e,i) => {
        e.y += e.spd; ctx.fillStyle='red'; ctx.beginPath(); ctx.arc(e.x+15,e.y+15,15,0,Math.PI*2); ctx.fill();
...
function gameOver() {
    gameState = 'OVER'; document.getElementById('game-hud').classList.add('hidden');
    document.getElementById('game-over-screen').classList.remove('hidden');
    document.getElementById('final-score').innerText = Math.floor(score);
    let s = JSON.parse(localStorage.getItem(`s_${poornataId}`) || '[]');
    s.push(Math.floor(score)); s.sort((a,b)=>b-a);
    localStorage.setItem(`s_${poornataId}`, JSON.stringify(s));
...
function start(n, id) {
    ...
    score=0; level=1; enemies=[]; powerups=[]; player=new Player(); animate();
}

// Controls
document.getElementById('pause-btn').addEventListener('click', () => { 
    isPaused = true; document.getElementById('pause-overlay').classList.remove('hidden'); 
});
document.getElementById('resume-btn').addEventListener('click', () => { 
    isPaused = false; document.getElementById('pause-overlay').classList.add('hidden'); animate(); 
});
...
document.getElementById('continue-btn').addEventListener('click', () => { 
    document.getElementById('level-modal').classList.add('hidden'); gameState='PLAYING'; animate(); 
});
